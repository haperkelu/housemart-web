...
import org.housemart.util.ZrHouseHelper.ZrHouseDetail;
...
public class HouseModuleDaoTester {
    ...
    @SuppressWarnings("rawtypes")
    @Autowired
    GenericDao zrHouseDao;

    @Test
    public void addHouseTest() {
	HouseEntity house = new HouseEntity();
	house.setName("test");
	System.out.println("Insert Test. House id : "
	...
	System.out.println("Load House Test. House name "
		+ ((HouseEntity) house).getName());
	;

	house = houseDao.load("loadHouseExt", 1);
	System.out.println("Load House Ext Test. House name "
		+ ((HouseEntity) house).getName());
	;
    }
	...
	rent.setMemo("abc");
	System.out.println("Insert HouseRent Test. HouseRent id : "
	...
	sale.setMemo("abc");
	System.out.println("Insert HouseSale Test. HouseSale id : "
	...
	List<HouseProcessEntity> houseProcessList = (List<HouseProcessEntity>) houseProcessDao
		.select("findLatestHouseProcessList", para);
	System.out.println("Find Latest House ProcessList Test. Size : "
		+ houseProcessList.size());
    }

    @Test
    public void findZrHouseTest() {

	for (int id = 1; id < 2000; id++) {
	    ZrHouse zrHouse = (ZrHouse) zrHouseDao.load("queryById", id);
	    if (zrHouse == null) {
		continue;
	    }
	    String detailString = zrHouse.getHouseDetail();
	    ZrHouseDetail zrDetail = ZrHouseHelper
		    .parseZrHouseDetail(detailString);
	    System.out.println(zrDetail);
	}
    }
