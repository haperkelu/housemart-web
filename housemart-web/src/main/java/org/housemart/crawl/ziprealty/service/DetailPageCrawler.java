...
public class DetailPageCrawler extends _ACrawler {
  ...
  public ZrHouse crawlDetailInfo(String detailUrl) throws JsonGenerationException, JsonMappingException, IOException {
    ...
    scraper.getContext().put("detail", status);
    scraper.getContext().put("status", status);
    ...
    if (!CollectionUtils.isEmpty(title)) {
      ...
      zrHouse.setMls(mls.get(0).toString());
      ...
      zrHouse.setNeighborhood(neighborhood.size() > 0 ? neighborhood.get(0).toString() : "");
      ...
      if (zrHouse.getMls() != null) {
        ...
        if (picUrlsOnZr != null) {
          ...
          for (String puOnZr : picUrlsOnZr) {
            ...
            String photoName = StringUtils.substringAfterLast(puOnZr, "/");
            String fileSavePath = new SimpleDateFormat("yyyyMMdd").format(new Date()) + _IZrConstants.FILE_SUBPATH_RESIDENCE + "/"
                + photoName;
            try {
